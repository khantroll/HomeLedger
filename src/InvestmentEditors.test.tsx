...
const eurSecurity:Security={id:"eur",name:"Euro Example",symbol:"EURX",securityType:"stock",currency:"EUR",archived:false};
...
describe("investment setup editors",()=>{
 ...
 it("locks account-scoped security creation to the investment account currency",async()=>{const view=render(<SecurityDialog account={account} onClose={()=>{}} onSaved={()=>{}}/>);fireEvent.change(field(view.container,"name"),{target:{value:"USD Security"}});const currency=field(view.container,"currency") as HTMLInputElement;expect(currency.readOnly).toBe(true);expect(currency.value).toBe("USD");fireEvent.change(currency,{target:{value:"EUR"}});fireEvent.submit(view.container.querySelector("form")!);await waitFor(()=>expect(investmentRepository.createSecurity).toHaveBeenCalledWith(expect.objectContaining({currency:"USD"})));});
 it("offers only securities matching the current investment account currency",()=>{const view=render(<InvestmentActivityDialog account={account} accounts={[account,destination]} securities={[eurSecurity,security]} holdings={[holding]} onClose={()=>{}} onSaved={()=>{}}/>);const select=field(view.container,"security") as HTMLSelectElement;expect([...select.options].map(o=>o.value)).toContain("sec");expect([...select.options].map(o=>o.value)).not.toContain("eur");expect(select.value).toBe("sec");});
});
...
describe("Money-style activity mapping",()=>{
 ...
 it("keeps an explicitly known zero opening basis distinct from unknown basis",async()=>{const knownZero=await submitActivity("opening_position",{quantity:"1",basis:"0.00",acquisitionDate:""});expect(knownZero.grossCashMinor).toBe(0);const unknown=await submitActivity("opening_position",{quantity:"1",basis:"",acquisitionDate:""});expect(unknown.grossCashMinor).toBeUndefined();});
 it("rejects mismatched specific-lot quantities before creating a sale event",async()=>{const view=render(<InvestmentActivityDialog account={account} accounts={[account,destination]} securities={[security]} holdings={[holding]} onClose={()=>{}} onSaved={()=>{}}/>);fireEvent.change(field(view.container,"activity"),{target:{value:"sell"}});fireEvent.change(field(view.container,"quantity"),{target:{value:"2"}});fireEvent.change(field(view.container,"amount"),{target:{value:"75.00"}});fireEvent.click(screen.getByLabelText("Specific lots"));fireEvent.change(field(view.container,"lot-lot1"),{target:{value:"1"}});fireEvent.submit(view.container.querySelector("form")!);expect(await screen.findByText(/must exactly equal the shares being sold/)).toBeTruthy();expect(investmentRepository.createInvestmentEvent).not.toHaveBeenCalled();});
