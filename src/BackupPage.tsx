...
export function BackupPage({onRestored}:{onRestored:()=>Promise<void>}) {
  ...
  return <div className="backup-page">
    ...
    <section className="panel backup-card recovery-health">
      <DatabaseBackup size={25}/><div><h2>Automatic recovery</h2><p>HomeLedger keeps rotating local SQLite recovery snapshots in its private application-data folder and creates one before upgrading an older database schema. Recovery snapshots are database-only; they validate against the live managed attachment store rather than duplicating every attachment into each rotating snapshot. Use an encrypted portable backup for a complete database-plus-attachments restore.</p></div>
