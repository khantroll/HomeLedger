...
import { FileSpreadsheet, History, RotateCcw, Undo2, Upload, X } from "lucide-react";
import { buildPreview, parseDelimited, suggestMapping, type ColumnMapping, type ParsedTable } from "./csvImport";
...
export function ImportPage({accounts,transactions,onImported}:{accounts:Account[];transactions:Transaction[];onImported:()=>Promise<void>}) {
  ...
  async function chooseFile(event:ChangeEvent<HTMLInputElement>){
    ...
    try{const text=await file.text();if(/<OFX[>\s]/i.test(text)||/\.(ofx|qfx)$/i.test(file.name)){const parsed=parseOfx(text);setOfx(parsed);setQif(null);setTable(null);}else if(/^!Type:/im.test(text)||/\.qif$/i.test(file.name)){const parsed=parseQif(text);setQif(parsed);setOfx(null);setTable(null);}else{const parsed=parseDelimited(text);setTable(parsed);setOfx(null);setQif(null);setMapping(suggestMapping(parsed.headers));}setFileName(file.name);}
  ...
  function reset(){setTable(null);setOfx(null);setQif(null);setFileName("");setError("");setMessage("");}
      ...
      <div className="import-body">
        ...
        {table&&<>
          <div className="mapping-grid">
            ...
            <MappingSelect label="Date" value={mapping.date} headers={table.headers} onChange={date=>setMapping({...mapping,date})}/>
            <MappingSelect label="Description / payee" value={mapping.payee} headers={table.headers} onChange={payee=>setMapping({...mapping,payee})}/>
            <MappingSelect label="Signed amount" value={mapping.amount} headers={table.headers} onChange={amount=>setMapping({...mapping,amount})}/>
            <MappingSelect label="Debit (optional)" value={mapping.debit} headers={table.headers} onChange={debit=>setMapping({...mapping,debit})}/>
            <MappingSelect label="Credit (optional)" value={mapping.credit} headers={table.headers} onChange={credit=>setMapping({...mapping,credit})}/>
          </div>
