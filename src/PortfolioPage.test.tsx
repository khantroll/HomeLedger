...
describe("Investment account workspace",()=>{
 ...
 it("allows today's observation in the Today view",async()=>{
  ...
  render(<PortfolioPage accounts={accounts} focus={{accountId:"inv",securityId:"sec"}} onShowAll={()=>{}} onOpenSecurity={()=>{}}/>);await screen.findByText("Security detail");expect((screen.getByLabelText("Price observation date") as HTMLInputElement).max).toBe("2026-09-21");fireEvent.change(screen.getByLabelText("Manual price"),{target:{value:"2"}});fireEvent.submit(screen.getByRole("button",{name:"Update price"}).closest("form")!);await vi.waitFor(()=>expect(investmentRepository.addManualSecurityPrice).toHaveBeenCalledWith(expect.objectContaining({observedAt:"2026-09-21",priceE8:200000000})));
 });
