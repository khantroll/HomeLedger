...
import {BarChart3,CalendarRange,ChevronRight,Download,Tags} from "lucide-react";
...
export function ReportsPage({accounts,transactions,today=todayIso(),onOpenAccount}:{accounts:Account[];transactions:Transaction[];today?:string;onOpenAccount?:(accountId:string)=>void}){
  ...
  async function exportReport(){
    if(!result.report)return;setExporting(true);setExportError("");setExportNotice("");
    try{const accountLabel=accountId==="all"?`All ${currency} accounts`:availableAccounts.find(item=>item.id===accountId)?.name??"Missing account";const saved=await csvExportRepository.saveCsv(buildReportCsv({report:result.report,currency,accountLabel,fromDate,toDate}),exportFileName("HomeLedger-report",fromDate,toDate));if(saved)setExportNotice("Report CSV saved.");}
  ...
  return <div className="reports-page">
    <div className="report-view-tabs" role="tablist" aria-label="Report views">
      <button type="button" role="tab" className={view==="household"?"active":""} aria-selected={view==="household"} onClick={()=>setView("household")}>Household</button>
      <button type="button" role="tab" className={view==="investments"?"active":""} aria-selected={view==="investments"} onClick={()=>setView("investments")}>Investments</button>
    </div>
    {view==="investments"?<InvestmentReportPanel accounts={accounts} today={today}/>:<>
    <section className="panel reports-header"><div><h2>Income and spending reports</h2><p>Deterministic local totals with direct access to every contributing ledger entry.</p></div><div className="reports-controls"><label>Currency<select value={currency} onChange={event=>{setCurrency(event.target.value);setSelectedKey("");}}>{currencies.length?currencies.map(item=><option key={item}>{item}</option>):<option>USD</option>}</select></label><label>Account<select value={accountId} onChange={event=>{setAccountId(event.target.value);setSelectedKey("");}}><option value="all">All {currency} accounts</option>{availableAccounts.map(item=><option key={item.id} value={item.id}>{item.name}</option>)}</select></label><label>Period<select value={preset} onChange={event=>choosePreset(event.target.value as ReportPreset)}><option value="month">This month</option><option value="quarter">Last 3 months</option><option value="year">Year to date</option><option value="twelve">Last 12 months</option><option value="custom">Custom range</option></select></label><button className="report-export" disabled={!result.report||exporting} onClick={()=>void exportReport()}><Download size={13}/>{exporting?"Preparing…":"Export CSV"}</button></div></section>
    ...
    {exportError&&<div className="error-banner" role="alert">{exportError}</div>}{exportNotice&&<div className="success-banner" role="status">{exportNotice}</div>}
    {!result.report?null:<>
      ...
      {selected&&<Drilldown group={selected} grouping={grouping} accounts={accounts} transactions={transactions} currency={currency} onOpenAccount={onOpenAccount}/>}
      <p className="report-note">Reports are calculated locally from saved ledger entries. Transfers and reconciliation adjustments are excluded; pending entries remain included until removed or changed in the register. Investment accounts are reported separately.</p>
    </>}
    </>}
  </div>;
}
...
function Drilldown({group,grouping,accounts,transactions,currency,onOpenAccount}:{group:ReportGroup;grouping:"category"|"payee";accounts:Account[];transactions:Transaction[];currency:string;onOpenAccount?:(accountId:string)=>void}){
