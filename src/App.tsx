...
import { ArrowLeftRight, BarChart3, Bot, CalendarDays, CircleDollarSign, FileInput, Landmark, LayoutDashboard, ListFilter, LockKeyhole, Menu, ReceiptText, Search, Settings, Tags, TrendingUp, WalletCards, X } from "lucide-react";
...
export default function App() {
  ...
  return <div className="app-shell">
    <aside className="sidebar">
      ...
      <nav aria-label="Primary navigation">{navItems.map(([label, Icon]) => <button key={label} className={active === label ? "active" : ""} onClick={() => setActive(label)}><Icon size={17}/><span>{label}</span>{!["Overview","Accounts","Transactions","Imports","Rules","Budget","Bills","Forecast","Debt","Reports","Settings"].includes(label) && <em>Planned</em>}</button>)}</nav>
    ...
    <main>
      ...
      <section className="content">
        {active === "Imports" ? <ImportPage accounts={accounts} transactions={transactions} onImported={refresh}/> : active === "Rules" ? <RulesPage/> : active === "Budget" ? <BudgetPage transactions={transactions} accounts={accounts}/> : active === "Bills" ? <BillsPage accounts={accounts} transactions={transactions} templates={scheduledTemplates} occurrences={scheduledOccurrences} onChanged={refresh}/> : active === "Forecast" ? <ForecastPage accounts={accounts} templates={scheduledTemplates}/> : active === "Debt" ? <DebtPage accounts={accounts}/> : active === "Reports" ? <ReportsPage accounts={accounts} transactions={transactions}/> : active === "Settings" ? <BackupPage onRestored={refresh}/> : active !== "Overview" && active !== "Accounts" && active !== "Transactions" ? <Planned title={active}/> : <>
