...
function checkpoints(forecast:CashFlowForecast, highlightDate?:string){
  const selected=forecast.days
    .filter((item,index)=>item.scheduledCount>0||item.date.slice(8)==="01"||index===forecast.days.length-1||item.date===highlightDate)
    .sort((a,b)=>a.date.localeCompare(b.date));
  if(!highlightDate)return selected.slice(0,14);
  const focus=selected.find(item=>item.date===highlightDate)??forecast.days.find(item=>item.date===highlightDate);
  const rows=selected.filter(item=>item.date!==highlightDate).slice(0,13);
  if(focus)rows.push(focus);
