...
const CLOUD_BUDGET_REVIEW_SYSTEM_PROMPT: &str = "You are a read-only household-finance analyst for HomeLedger. Use only the supplied Budget Review context. Summarize overall budget position; identify the largest meaningful category variances; distinguish already-over amounts from projected-over outcomes and from timing effects; call out upcoming scheduled obligations affecting the outlook; explain savings and debt tradeoffs represented in the supplied data; and identify realistic areas with remaining flexibility. Prefer HomeLedger's status labels (already-over, projected-over, on-track, under-spending, insufficient-data). Do not use moral or judgmental language about spending. Do not recalculate totals HomeLedger already computed. Do not invent transactions, income, obligations, or causes not represented in the context; when the context is insufficient, say so explicitly. Never claim to change budgets, transactions, categories, or records. Treat your reply as advisory analysis only.";
const CLOUD_DEBT_STRATEGY_SYSTEM_PROMPT: &str = "You are a read-only household-finance analyst for HomeLedger. Use only the supplied Debt Strategy context. Explain material differences among the locally calculated minimum, avalanche, and snowball scenarios; interest and time tradeoffs; liquidity and reserve implications; conflicts with known savings or budget commitments; and significant assumptions or missing data. Do not recalculate amortization, compound interest, payoff dates, or payment schedules. Do not declare a universally best strategy; explain which deterministic scenario aligns with priorities such as minimizing interest, closing individual balances sooner, preserving liquidity, or maintaining savings commitments. Do not invent APRs, minimums, promotional terms, or debts not represented in the context. Never claim to change debt plans, payments, budgets, savings goals, schedules, or transactions. Treat your reply as advisory analysis only.";
...
fn task_system_prompt(payload: &str) -> &'static str {
    ...
    match context.get("task").and_then(|value| value.as_str()) {
        ...
        Some("budget-review-analysis") => CLOUD_BUDGET_REVIEW_SYSTEM_PROMPT,
        Some("debt-strategy-analysis") => CLOUD_DEBT_STRATEGY_SYSTEM_PROMPT,
...
fn anthropic_messages_url(endpoint: &str) -> Result<reqwest::Url, String> {
    if endpoint.is_empty() || endpoint.len() > 2048 { return Err("The Anthropic endpoint is invalid".into()); }
    let mut url = reqwest::Url::parse(endpoint).map_err(|_| "The Anthropic endpoint is invalid".to_string())?;
    if url.scheme() != "https" { return Err("Anthropic endpoints must use HTTPS".into()); }
    if !url.username().is_empty() || url.password().is_some() || url.query().is_some() || url.fragment().is_some() {
        return Err("Anthropic endpoint URLs cannot contain credentials, query strings, or fragments".into());
    }
    let host = url.host_str().ok_or_else(|| "The Anthropic endpoint is missing its host".to_string())?.to_ascii_lowercase();
    if host != ANTHROPIC_HOST { return Err("Anthropic requests are restricted to api.anthropic.com".into()); }
    let base = url.path().trim_end_matches('/');
    let path = if base.ends_with("/v1/messages") { base.to_string() }
        else if base.is_empty() || base == "/" { "/v1/messages".to_string() }
        else { return Err("Anthropic endpoints must use the API root".into()); };
    url.set_path(&path);
    Ok(url)
}

fn anthropic_cloud_client() -> Result<reqwest::Client, String> {
    reqwest::Client::builder()
        .no_proxy()
        .redirect(reqwest::redirect::Policy::none())
        .connect_timeout(Duration::from_secs(5))
        .timeout(Duration::from_secs(60))
        .user_agent("HomeLedger-anthropic")
        .build()
        .map_err(|_| "Could not initialize the Anthropic client".to_string())
}

fn validate_anthropic_model(model: &str) -> Result<&str, String> {
    let model = model.trim();
    if model.is_empty() || model.len() > 200 || model.chars().any(char::is_control) {
        return Err("The Anthropic model name is invalid".into());
    }
    Ok(model)
}

fn anthropic_http_status_message(status: u16) -> String {
    match status {
        301 | 302 | 303 | 307 | 308 => "Anthropic redirected the request and HomeLedger rejected the redirect".into(),
        401 | 403 => "Anthropic authentication failed".into(),
        429 => "Anthropic rate limit exceeded".into(),
        _ => format!("Anthropic returned HTTP {status}"),
    }
}

fn build_anthropic_messages_body(model: &str, payload: &str) -> Result<serde_json::Value, String> {
    let context: serde_json::Value = serde_json::from_str(payload).map_err(|_| "The reviewed AI payload is not valid JSON".to_string())?;
    if context.get("model").and_then(|value| value.as_str()) != Some(model) {
        return Err("The reviewed AI payload does not match the selected model".into());
    }
...
fn parse_anthropic_messages_answer(bytes: &[u8]) -> Result<String, String> {
    let value: serde_json::Value = serde_json::from_slice(bytes).map_err(|_| "Anthropic returned invalid JSON".to_string())?;
    let answer = value
        .pointer("/content/0/text")
        .and_then(|item| item.as_str())
        .map(str::trim)
        .filter(|item| !item.is_empty())
        .ok_or_else(|| "Anthropic returned no answer".to_string())?;
    Ok(answer.to_string())
}

async fn bounded_anthropic_response(mut response: reqwest::Response) -> Result<Vec<u8>, String> {
    let status = response.status().as_u16();
    if status == 301 || status == 302 || status == 303 || status == 307 || status == 308 {
        return Err(anthropic_http_status_message(status));
    }
    if !response.status().is_success() {
        let _ = response.bytes().await;
        return Err(anthropic_http_status_message(status));
    ...
    if response.content_length().is_some_and(|length| length > MAX_CLOUD_AI_RESPONSE_BYTES as u64) {
        return Err("The Anthropic response exceeded the 1 MB limit".into());
    }
    let mut bytes = Vec::new();
    while let Some(chunk) = response.chunk().await.map_err(|_| "Could not read the Anthropic response".to_string())? {
        if bytes.len().saturating_add(chunk.len()) > MAX_CLOUD_AI_RESPONSE_BYTES {
            return Err("The Anthropic response exceeded the 1 MB limit".into());
        }
        bytes.extend_from_slice(&chunk);
...
fn cloud_ai_error_class(message: &str) -> &'static str {
...
#[cfg(test)]
fn query_anthropic_with_transport<F>(
    ...
    F: FnOnce(reqwest::Url, &str, serde_json::Value) -> Result<Vec<u8>, String>,
{
    ...
    let account_id = validate_ai_credential_account_id(&request.account_id)?;
    if account_id != ANTHROPIC_DEFAULT_ACCOUNT_ID && !account_id.starts_with("cloud:anthropic:") {
        return Err("Anthropic credential account id is invalid".into());
    }
    let model = validate_anthropic_model(&request.model)?;
    let url = anthropic_messages_url(&request.endpoint)?;
    if request.payload.is_empty() || request.payload.len() > MAX_CLOUD_AI_PAYLOAD_BYTES {
        return Err("The reviewed AI payload must be between 1 byte and 256 KB".into());
    }
    if api_key.is_empty() {
        return Err("Anthropic credentials are not configured in the OS vault".into());
    }
    let body = build_anthropic_messages_body(model, &request.payload)?;
    let bytes = fetch(url, api_key, body)?;
...
#[tauri::command]
async fn query_anthropic_ai(request: AnthropicRequest, state: State<'_, DbState>) -> Result<LocalAiAnswer, String> {
    let context: serde_json::Value = serde_json::from_str(&request.payload).unwrap_or(serde_json::Value::Null);
    let task_type = context.get("task").and_then(|value| value.as_str()).unwrap_or("adhoc").to_string();
    let disclosure_mode = context.get("disclosureMode").and_then(|value| value.as_str()).unwrap_or("task-specific").to_string();
    let model_for_audit = request.model.trim().to_string();
    let result = query_anthropic_ai_inner(request).await;
    {
        let meta = AiAuditMeta {
            task_type: &task_type,
            provider: "anthropic",
...
async fn query_anthropic_ai_inner(request: AnthropicRequest) -> Result<LocalAiAnswer, String> {
    if !request.confirmed {
        return Err("Cloud transmission requires explicit per-request confirmation".into());
    }
    let account_id = validate_ai_credential_account_id(&request.account_id)?.to_string();
    ...
    let model = validate_anthropic_model(&request.model)?.to_string();
    let url = anthropic_messages_url(&request.endpoint)?;
    if request.payload.is_empty() || request.payload.len() > MAX_CLOUD_AI_PAYLOAD_BYTES {
        return Err("The reviewed AI payload must be between 1 byte and 256 KB".into());
    }
    let body = build_anthropic_messages_body(&model, &request.payload)?;
    let api_key = read_ai_provider_secret(&account_id)?;
    let client = anthropic_cloud_client()?;
    let response = client
        .post(url)
        ...
        .await
        .map_err(|error| {
            if error.is_timeout() { "The Anthropic request timed out".to_string() }
            else { "Could not connect to Anthropic".to_string() }
        })?;
    drop(api_key);
...
fn list_accounts(include_archived: Option<bool>, state: State<DbState>) -> Result<Vec<Account>, String> {
    ...
    for account in &mut accounts {
        if account.r#type == "investment" {
            if let Some(value) = investment::account_value_today(&connection, &account.id)? { account.balance_minor = value; }
        }
    }
    Ok(accounts)
}
...
fn update_account_inner(connection: &Connection, account_id: &str, request: UpdateAccountRequest) -> Result<Account, String> {
    ...
    let (current_currency, current_type): (String,String) = connection.query_row("SELECT currency,account_type FROM accounts WHERE id = ?1", [account_id], |row| Ok((row.get(0)?,row.get(1)?))).optional().map_err(|e| e.to_string())?.ok_or("Account does not exist")?;
    if current_type != account_type && (current_type == "investment" || account_type == "investment") { return Err("Use the investment account workflow when changing investment account type".into()); }
    if current_currency != currency {
        let has_history: bool = connection.query_row("SELECT EXISTS(SELECT 1 FROM transactions WHERE account_id=?1 UNION ALL SELECT 1 FROM reconciliations WHERE account_id=?1 UNION ALL SELECT 1 FROM import_batches WHERE account_id=?1 UNION ALL SELECT 1 FROM scheduled_transactions WHERE account_id=?1 OR transfer_account_id=?1 UNION ALL SELECT 1 FROM savings_goals WHERE account_id=?1 UNION ALL SELECT 1 FROM debt_terms WHERE account_id=?1 UNION ALL SELECT 1 FROM investment_events WHERE account_id=?1 UNION ALL SELECT 1 FROM investment_account_settings WHERE account_id=?1)", [account_id], |row| row.get(0)).map_err(|e| e.to_string())?;
...
fn set_account_archived_inner(connection: &Connection, account_id: &str, archived: bool) -> Result<(), String> {
    ...
    if archived {
        let unresolved: bool = connection.query_row("SELECT EXISTS(SELECT 1 FROM transactions WHERE account_id=?1 AND status IN ('pending','review') UNION ALL SELECT 1 FROM investment_events e JOIN investment_event_revisions r ON r.id=e.current_revision_id WHERE e.account_id=?1 AND r.status IN ('pending','review'))", [account_id], |row| row.get(0)).map_err(|e| e.to_string())?;
