...
fn safe_json_int(value:i64)->bool{value.unsigned_abs()<=MAX_SAFE_JSON_INTEGER}
...
#[tauri::command] pub fn get_investment_account_settings(account_id:String,state:State<DbState>)->Result<Option<InvestmentAccountSettings>,String>{let c=state.0.lock().map_err(|_|"Database lock failed")?;get_settings_inner(&c,&account_id)}
#[tauri::command] pub fn save_investment_account_settings(request:InvestmentAccountSettingsRequest,state:State<DbState>)->Result<InvestmentAccountSettings,String>{let c=state.0.lock().map_err(|_|"Database lock failed")?;upsert_settings_inner(&c,request)}
...
fn validate_event(c:&Connection,r:&mut InvestmentEventRequest)->Result<(),String>{
 ...
 if [r.cash_effect_minor,r.income_minor,r.acquisition_funding_minor,r.fee_minor,r.basis_effect_minor].iter().any(|v|!safe_json_int(*v))||r.quantity_e8.is_some_and(|v|!safe_json_int(v))||r.unit_price_e8.is_some_and(|v|!safe_json_int(v)){return Err("Investment numeric value exceeds the exact application transport range".into())}
 ...
 if matches!(r.event_type.as_str(),"cash_transfer"|"security_transfer"){let id=r.related_account_id.as_ref().unwrap();if id==&r.account_id{return Err("Transfer destination must differ from source".into())}let (dest_currency,dest_type):(String,String)=c.query_row("SELECT currency,account_type FROM accounts WHERE id=?1",[id],|x|Ok((x.get(0)?,x.get(1)?))).optional().map_err(|e|e.to_string())?.ok_or("Transfer destination does not exist")?;let source_currency:String=c.query_row("SELECT currency FROM accounts WHERE id=?1",[&r.account_id],|x|x.get(0)).map_err(|e|e.to_string())?;if dest_currency!=source_currency{return Err("Foundation transfers do not perform FX conversion".into())}if r.event_type=="security_transfer"&&dest_type!="investment"{return Err("Security transfers require an investment destination account".into())}}
 Ok(())
}
...
#[tauri::command] pub fn create_investment_event(request:InvestmentEventRequest,state:State<DbState>)->Result<InvestmentEventRevision,String>{let mut c=state.0.lock().map_err(|_|"Database lock failed")?;create_event_inner(&mut c,request)}
...
fn consume(lots:&mut Vec<Lot>,qty:i64,specific:&[LotAllocation],proceeds:i64)->Result<RealizedResult,String>{
 ...
 else{for a in specific{let i=lots.iter().position(|l|l.event_id==a.acquisition_event_id).ok_or("Specific lot is not open")?;if a.quantity_e8>lots[i].quantity_e8{return Err("Specific-lot allocation exceeds open lot".into())};needs.push((i,a.quantity_e8));}}
...
fn move_fifo_lots(lots:&mut Vec<Lot>,qty:i64)->Result<Vec<Lot>,String>{
 let available:i64=lots.iter().map(|l|l.quantity_e8).sum();if qty>available{return Err("Security transfer exceeds available units".into())}
 let mut left=qty;let mut moved=vec![];
 for lot in lots.iter_mut(){if left==0{break}let take=left.min(lot.quantity_e8);if take<=0{continue}let moved_basis=match lot.basis_minor{Some(b)=>{let part=if take==lot.quantity_e8{b}else{proportional(b,take,lot.quantity_e8)?};lot.basis_minor=Some(b-part);Some(part)},None=>None};moved.push(Lot{event_id:lot.event_id.clone(),acquisition_date:lot.acquisition_date.clone(),quantity_e8:take,basis_minor:moved_basis});lot.quantity_e8-=take;left-=take;}
 lots.retain(|l|l.quantity_e8>0);Ok(moved)
}
...
pub fn snapshot_inner(c:&Connection,account_ids:Option<&[String]>,as_of:&str)->Result<PortfolioSnapshot,String>{
 ...
 for e in all{
  ...
  match e.event_type.as_str(){
   "buy"|"reinvest_dividend"=>{lotmap.entry(key).or_default().push(Lot{event_id:e.event_id.clone(),acquisition_date:e.acquisition_date.clone().unwrap_or(e.trade_date.clone()),quantity_e8:e.quantity_e8.unwrap(),basis_minor:Some(e.acquisition_funding_minor.checked_add(e.fee_minor).ok_or("Investment basis overflow")?)});}
   "opening_position"=>{lotmap.entry(key).or_default().push(Lot{event_id:e.event_id.clone(),acquisition_date:e.acquisition_date.clone().unwrap_or(e.trade_date.clone()),quantity_e8:e.quantity_e8.unwrap(),basis_minor:e.gross_cash_minor});}
   "sell"=>{let a=allocations(c,&e.id)?;let rr=consume(lotmap.entry(key.clone()).or_default(),e.quantity_e8.unwrap(),&a,e.cash_effect_minor)?;let cur=realized.entry(key).or_default();cur.known_basis_quantity_e8+=rr.known_basis_quantity_e8;cur.known_disposed_basis_minor+=rr.known_disposed_basis_minor;cur.calculable_proceeds_minor+=rr.calculable_proceeds_minor;cur.calculable_gain_minor+=rr.calculable_gain_minor;cur.unknown_basis_quantity_e8+=rr.unknown_basis_quantity_e8;cur.unknown_basis_proceeds_minor+=rr.unknown_basis_proceeds_minor;cur.incomplete_unknown_basis|=rr.incomplete_unknown_basis;}
   "split"=>{let n=e.split_numerator.unwrap();let d=e.split_denominator.unwrap();for l in lotmap.entry(key).or_default(){let q=(l.quantity_e8 as i128)*(n as i128)/(d as i128);l.quantity_e8=i64::try_from(q).map_err(|_|"Split quantity overflow")?;}}
   "return_of_capital"|"basis_adjustment"=>{let lots=lotmap.entry(key).or_default();let known_total:i64=lots.iter().filter_map(|l|l.basis_minor).sum();let known_indices:Vec<usize>=lots.iter().enumerate().filter(|(_,l)|l.basis_minor.is_some()).map(|(i,_)|i).collect();if !known_indices.is_empty(){if e.basis_effect_minor<0&&-e.basis_effect_minor>known_total{return Err("Basis adjustment would drive known basis below zero".into())}let mut remaining=e.basis_effect_minor;for(pos,i)in known_indices.iter().enumerate(){let b=lots[*i].basis_minor.unwrap();let delta=if pos+1==known_indices.len(){remaining}else{proportional(e.basis_effect_minor,b,known_total.max(1))?};lots[*i].basis_minor=Some(b+delta);remaining-=delta;}}else if e.event_type=="basis_adjustment"{return Err("Cannot apply an exact basis adjustment when all open basis is unknown".into())}}
   "security_transfer"=>{let qty=e.quantity_e8.unwrap();let moved=move_fifo_lots(lotmap.entry(key).or_default(),qty)?;let dest=e.related_account_id.clone().unwrap();lotmap.entry((dest,sec)).or_default().extend(moved);}
   _=>{}
  }
 }
...
#[tauri::command] pub fn calculate_portfolio_snapshot(account_ids:Option<Vec<String>>,as_of_date:String,state:State<DbState>)->Result<PortfolioSnapshot,String>{let c=state.0.lock().map_err(|_|"Database lock failed")?;snapshot_inner(&c,account_ids.as_deref(),&as_of_date)}
#[tauri::command] pub fn derive_investment_holdings(account_id:String,as_of_date:String,state:State<DbState>)->Result<Vec<HoldingSnapshot>,String>{let c=state.0.lock().map_err(|_|"Database lock failed")?;let p=snapshot_inner(&c,Some(&[account_id.clone()]),&as_of_date)?;Ok(p.accounts.into_iter().find(|a|a.account_id==account_id).map(|a|a.holdings).unwrap_or_default())}
...
#[tauri::command] pub fn list_security_prices(security_id:String,from_date:Option<String>,to_date:Option<String>,state:State<DbState>)->Result<Vec<SecurityPrice>,String>{if let Some(v)=&from_date{date(v,"From date")?}if let Some(v)=&to_date{date(v,"To date")?}let c=state.0.lock().map_err(|_|"Database lock failed")?;let mut s=c.prepare("SELECT id,security_id,observed_at,price_e8,currency,source,provenance FROM security_prices WHERE security_id=?1 AND (?2 IS NULL OR substr(observed_at,1,10)>=?2) AND (?3 IS NULL OR substr(observed_at,1,10)<=?3) ORDER BY observed_at,id").map_err(|e|e.to_string())?;let rows=s.query_map(params![security_id,from_date,to_date],|r|Ok(SecurityPrice{id:r.get(0)?,security_id:r.get(1)?,observed_at:r.get(2)?,price_e8:r.get(3)?,currency:r.get(4)?,source:r.get(5)?,provenance:r.get(6)?})).map_err(|e|e.to_string())?;let out=rows.collect::<Result<Vec<_>,_>>().map_err(|e|e.to_string())?;Ok(out)}
...
mod tests {
 ...
 fn db()->Connection{let mut c=Connection::open_in_memory().unwrap();apply_migrations(&mut c).unwrap();c.execute("INSERT INTO accounts(id,name,account_type,currency,opening_balance_minor,owner_label) VALUES('inv','Brokerage','investment','USD',0,'Household')",[]).unwrap();c.execute("INSERT INTO investment_account_settings(account_id,account_kind,tax_treatment,opening_cash_minor,opening_date) VALUES('inv','brokerage','taxable',100000,'2026-01-01')",[]).unwrap();c.execute("INSERT INTO securities(id,security_type,name,symbol,currency) VALUES('sec','stock','Example','EX','USD')",[]).unwrap();c}
 fn event(kind:&str,date:&str,qty:Option<i64>,cash:i64,funding:i64,status:&str)->InvestmentEventRequest{InvestmentEventRequest{account_id:"inv".into(),event_type:kind.into(),trade_date:date.into(),settlement_date:None,acquisition_date:None,security_id:if matches!(kind,"interest"|"fee"|"cash_transfer"){None}else{Some("sec".into())},related_account_id:None,quantity_e8:qty,unit_price_e8:None,gross_cash_minor:None,cash_effect_minor:cash,income_minor:0,acquisition_funding_minor:funding,fee_minor:0,basis_effect_minor:0,split_numerator:None,split_denominator:None,status:status.into(),source:"manual".into(),memo:None,external_id:None,provenance:None,group_id:None}}
 #[test] fn migration_adds_portfolio_foundation_schema(){let c=db();let version:i64=c.query_row("SELECT MAX(version) FROM schema_migrations",[],|r|r.get(0)).unwrap();let tables:i64=c.query_row("SELECT COUNT(*) FROM sqlite_master WHERE type='table' AND name IN ('investment_account_settings','securities','security_identifiers','investment_events','investment_event_revisions','investment_lot_allocations','security_prices')",[],|r|r.get(0)).unwrap();assert_eq!((version,tables),(18,7));}
 #[test] fn fifo_partial_unknown_basis_reports_incomplete_gain(){let mut c=db();create_event_inner(&mut c,event("buy","2026-01-02",Some(SCALE_E8),-10000,10000,"cleared")).unwrap();let mut opening=event("opening_position","2026-01-03",Some(SCALE_E8),0,0,"cleared");opening.gross_cash_minor=None;create_event_inner(&mut c,opening).unwrap();create_event_inner(&mut c,event("sell","2026-02-01",Some(SCALE_E8+SCALE_E8/2),18000,0,"cleared")).unwrap();let p=snapshot_inner(&c,Some(&["inv".into()]),"2026-02-01").unwrap();let r=&p.accounts[0].holdings[0].realized;assert_eq!(r.known_basis_quantity_e8,SCALE_E8);assert_eq!(r.known_disposed_basis_minor,10000);assert_eq!(r.unknown_basis_quantity_e8,SCALE_E8/2);assert!(r.incomplete_unknown_basis);assert_eq!(r.calculable_proceeds_minor,12000);assert_eq!(r.unknown_basis_proceeds_minor,6000);assert_eq!(r.calculable_gain_minor,2000);}
 #[test] fn reinvested_dividend_preserves_income_acquisition_and_zero_cash(){let mut c=db();let mut r=event("reinvest_dividend","2026-03-01",Some(SCALE_E8),0,10000,"cleared");r.income_minor=10000;let saved=create_event_inner(&mut c,r).unwrap();assert_eq!((saved.income_minor,saved.acquisition_funding_minor,saved.cash_effect_minor),(10000,10000,0));let p=snapshot_inner(&c,Some(&["inv".into()]),"2026-03-01").unwrap();assert_eq!(p.accounts[0].cash_minor,100000);assert_eq!(p.accounts[0].holdings[0].known_basis_minor,10000);}
 #[test] fn settlement_date_moves_cash_after_trade_date(){let mut c=db();let mut r=event("buy","2026-04-01",Some(SCALE_E8),-5000,5000,"cleared");r.settlement_date=Some("2026-04-03".into());create_event_inner(&mut c,r).unwrap();let trade=snapshot_inner(&c,Some(&["inv".into()]),"2026-04-01").unwrap();assert_eq!(trade.accounts[0].holdings[0].quantity_e8,SCALE_E8);assert_eq!(trade.accounts[0].cash_minor,100000);let settled=snapshot_inner(&c,Some(&["inv".into()]),"2026-04-03").unwrap();assert_eq!(settled.accounts[0].cash_minor,95000);}
 #[test] fn split_preserves_basis_and_changes_quantity(){let mut c=db();create_event_inner(&mut c,event("buy","2026-01-02",Some(SCALE_E8),-10000,10000,"cleared")).unwrap();let mut s=event("split","2026-05-01",None,0,0,"cleared");s.split_numerator=Some(2);s.split_denominator=Some(1);create_event_inner(&mut c,s).unwrap();let h=snapshot_inner(&c,Some(&["inv".into()]),"2026-05-01").unwrap().accounts.remove(0).holdings.remove(0);assert_eq!(h.quantity_e8,2*SCALE_E8);assert_eq!(h.known_basis_minor,10000);}
 #[test] fn protected_correction_keeps_prior_revision_and_recalculates(){let mut c=db();let first=create_event_inner(&mut c,event("buy","2026-01-02",Some(SCALE_E8),-10000,10000,"cleared")).unwrap();let mut replacement=event("buy","2026-01-02",Some(SCALE_E8),-12000,12000,"cleared");validate_event(&c,&mut replacement).unwrap();let tx=c.transaction().unwrap();let corrected=insert_revision(&tx,&first.event_id,2,Some(&first.id),&replacement,Some("Correct statement basis")).unwrap();tx.commit().unwrap();let count:i64=c.query_row("SELECT COUNT(*) FROM investment_event_revisions WHERE event_id=?1",[&first.event_id],|r|r.get(0)).unwrap();assert_eq!(count,2);assert_eq!(corrected.supersedes_revision_id.as_deref(),Some(first.id.as_str()));let h=snapshot_inner(&c,Some(&["inv".into()]),"2026-01-02").unwrap().accounts.remove(0).holdings.remove(0);assert_eq!(h.known_basis_minor,12000);}
 #[test] fn manual_prices_are_historical_and_missing_is_unknown(){let mut c=db();create_event_inner(&mut c,event("buy","2026-01-02",Some(SCALE_E8),-10000,10000,"cleared")).unwrap();c.execute("INSERT INTO security_prices(id,security_id,observed_at,price_e8,currency,source) VALUES('p1','sec','2026-01-10',15000000000,'USD','manual'),('p2','sec','2026-02-10',20000000000,'USD','manual')",[]).unwrap();let jan=snapshot_inner(&c,Some(&["inv".into()]),"2026-01-15").unwrap();assert_eq!(jan.accounts[0].holdings[0].market_value_minor,Some(15000));let feb=snapshot_inner(&c,Some(&["inv".into()]),"2026-02-15").unwrap();assert_eq!(feb.accounts[0].holdings[0].market_value_minor,Some(20000));let pre=snapshot_inner(&c,Some(&["inv".into()]),"2026-01-05").unwrap();assert_eq!(pre.accounts[0].holdings[0].market_value_minor,None);assert_eq!(pre.accounts[0].total_value_minor,None);}
 #[test] fn pending_edit_and_protected_history_rules_are_distinct(){let mut c=db();let pending=create_event_inner(&mut c,event("buy","2026-01-02",Some(SCALE_E8),-10000,10000,"pending")).unwrap();let status:String=c.query_row("SELECT status FROM investment_event_revisions WHERE id=?1",[pending.id],|r|r.get(0)).unwrap();assert_eq!(status,"pending");let cleared=create_event_inner(&mut c,event("buy","2026-01-03",Some(SCALE_E8),-10000,10000,"cleared")).unwrap();let status:String=c.query_row("SELECT status FROM investment_event_revisions WHERE id=?1",[cleared.id],|r|r.get(0)).unwrap();assert_eq!(status,"cleared");}
 #[test] fn exact_transport_range_rejects_unsafe_e8(){let c=db();let mut r=event("buy","2026-01-02",Some(9_007_199_254_740_992),-10000,10000,"pending");assert!(validate_event(&c,&mut r).unwrap_err().contains("transport range"));}
}
